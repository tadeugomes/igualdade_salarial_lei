...
<<<<<<< HEAD
    ...
    # Apply header formatting to the first row
    for col_num, col_name in enumerate(dist_cbo_detailed.columns):
        sheet_dist_detalhada.write(0, col_num, col_name, header_fmt)
    
    # Get unique CBOs for creating separate charts
    unique_cbos = dist_cbo_detailed[['cbo_2002', 'cbo_titulo']].drop_duplicates()
    
    # Create separate charts for each CBO showing distribution by sex and race/color
    start_row_chart = len(dist_cbo_detailed) + 3
    chart_height = 15  # Approximate height for chart positioning
    
    for idx, (cbo_code, cbo_title) in enumerate(zip(unique_cbos['cbo_2002'], unique_cbos['cbo_titulo'])):
        # Filter data for this specific CBO
        cbo_data = dist_cbo_detailed[dist_cbo_detailed['cbo_2002'] == cbo_code]
        
        # Create a pivot table for this CBO: Sex vs Race/Color
        ...
        # Write pivot table to the sheet
        pivot_start_row = start_row_chart + (idx * (len(cbo_pivot) + 5))
        cbo_pivot.to_excel(writer, sheet_name="Distribuicao_Detalhada", index=False, startrow=pivot_start_row)
        
        # Add header for pivot table
        for col_num, col_name in enumerate(cbo_pivot.columns):
            sheet_dist_detalhada.write(pivot_start_row, col_num, col_name, header_fmt)
        
        # Create chart for this CBO
        chart_cbo = workbook.add_chart({"type": "column", "subtype": "clustered"})
        
        # Add series for each race/color category (skip first column which is 'sexo')
        for i, col_name in enumerate(cbo_pivot.columns[1:], start=1):
            if cbo_pivot[col_name].sum() > 0:  # Only add series if there are values
                chart_cbo.add_series({
                    "name": col_name,
                    "categories": ["Distribuicao_Detalhada", pivot_start_row+1, 0, pivot_start_row+len(cbo_pivot), 0],  # Sex
                    "values": ["Distribuicao_Detalhada", pivot_start_row+1, i, pivot_start_row+len(cbo_pivot), i],
                })
        
        chart_cbo.set_title({"name": f"Distribuição de Trabalhadores - {cbo_title} ({cbo_code})"})
        chart_cbo.set_x_axis({"name": "Sexo"})
        chart_cbo.set_y_axis({"name": "Número de Trabalhadores"})
        chart_cbo.set_legend({"position": "bottom"})
        
        # Position chart below the pivot table
        chart_row = pivot_start_row + len(cbo_pivot) + 2
        sheet_dist_detalhada.insert_chart(f"A{chart_row}", chart_cbo, {"x_scale": 1.5, "y_scale": 1.5})
    
    # NEW: Add a separate worksheet with boxplots for salary distribution by CBO and sex
    # Create a new worksheet for boxplots
    sheet_boxplot = workbook.add_worksheet("Boxplots_Salarios")
    
    # Apply header formatting to the first row
    for col_num in range(15):  # Enough columns for our data
        sheet_boxplot.write(0, col_num, "", header_fmt)
    
    # Add title for the boxplot sheet
    title_format = workbook.add_format({
        "bold": True,
        "font_size": 14,
        "align": "center"
    })
    sheet_boxplot.merge_range("A1:O1", "Boxplots de Salários por CBO e Sexo", title_format)
    
    # Get unique CBOs
    unique_cbos = df[['cbo_2002', 'cbo_titulo']].drop_duplicates().reset_index(drop=True)
    
    # Define colors for F and M (Purple for F, Yellow for M as requested)
    female_color = "#9966CC"  # Purple
    male_color = "#FFD700"    # Yellow
    
    # Add data for boxplots to the worksheet
    data_start_row = 3
    chart_start_col = 0
    data_start_col = 8  # Leave space for charts on the left
    
    # Write column headers for the data
    data_headers = ["CBO", "Título", "Sexo", "Salário", "Q1", "Q2 (Mediana)", "Q3", "Min", "Max"]
    for col_num, header in enumerate(data_headers):
        sheet_boxplot.write(data_start_row - 1, data_start_col + col_num, header, header_fmt)
    
    # Add boxplots for each CBO
    row_offset = data_start_row
    chart_row_offset = 3
    
    for idx, (cbo_code, cbo_title) in enumerate(zip(unique_cbos['cbo_2002'], unique_cbos['cbo_titulo'])):
        # Filter data for this specific CBO
        cbo_data = df[df['cbo_2002'] == cbo_code].copy()
        
        # Calculate statistics for boxplot
        stats_data = []
        for sex in ['F', 'M']:
            sex_data = cbo_data[cbo_data['sexo'] == sex]['salario_contratual_mensal']
            if not sex_data.empty:
                q1 = sex_data.quantile(0.25)
                q2 = sex_data.quantile(0.5)  # Median
                q3 = sex_data.quantile(0.75)
                min_val = sex_data.min()
                max_val = sex_data.max()
                
                # Add individual data points for scatter plot
                for salary in sex_data:
                    stats_data.append([
                        cbo_code,
                        cbo_title,
                        sex,
        ...
        # Write statistics data to worksheet
        for i, data_row in enumerate(stats_data):
            for j, value in enumerate(data_row):
                sheet_boxplot.write(row_offset + i, data_start_col + j, value)
        
        # Create a custom boxplot using line and column charts
        chart_box = workbook.add_chart({"type": "line"})
        
        # Add data series for each sex
        female_start = row_offset
        female_count = sum(1 for row in stats_data if row[2] == 'F')
        male_start = female_start + female_count
        male_count = sum(1 for row in stats_data if row[2] == 'M')
        
        # Add female data points
        if female_count > 0:
            chart_box.add_series({
                "name": "Mulheres (F)",
                "categories": ["Boxplots_Salarios", female_start, data_start_col + 1, 
                               female_start + female_count - 1, data_start_col + 1],
                "values": ["Boxplots_Salarios", female_start, data_start_col + 3, 
                           female_start + female_count - 1, data_start_col + 3],
                "marker": {"type": "circle", "size": 4, "color": female_color},
                "line": {"color": female_color, "width": 1.5},
            })
        
        # Add male data points
        if male_count > 0:
            chart_box.add_series({
                "name": "Homens (M)",
                "categories": ["Boxplots_Salarios", male_start, data_start_col + 1, 
                               male_start + male_count - 1, data_start_col + 1],
                "values": ["Boxplots_Salarios", male_start, data_start_col + 3, 
                           male_start + male_count - 1, data_start_col + 3],
                "marker": {"type": "circle", "size": 4, "color": male_color},
                "line": {"color": male_color, "width": 1.5},
            })
        
        chart_box.set_title({"name": f"Salários - {cbo_title} ({cbo_code})"})
        chart_box.set_x_axis({"name": "Trabalhadores"})
        chart_box.set_y_axis({"name": "Salário Contratual Mensal (R$)"})
        chart_box.set_legend({"position": "bottom"})
        chart_box.set_style(10)
        
        # Insert chart
        chart_position = f"{chr(65 + chart_start_col)}{chart_row_offset}"
        sheet_boxplot.insert_chart(chart_position, chart_box, {"x_scale": 2, "y_scale": 1.5})
        
        # Add median value labels
        # Find medians for display
        female_median = None
        male_median = None
        for row in stats_data:
            if row[2] == 'F' and female_median is None:
                female_median = row[5]  # Q2 column
            elif row[2] == 'M' and male_median is None:
                male_median = row[5]  # Q2 column
        
        # Add median labels
        label_row = chart_row_offset - 1
        if female_median is not None:
            sheet_boxplot.write(label_row, chart_start_col + 2, 
                               f"Mediana Mulheres: R${female_median:.2f}", 
                               workbook.add_format({"color": female_color, "bold": True}))
        
        if male_median is not None:
            sheet_boxplot.write(label_row, chart_start_col + 4, 
                               f"Mediana Homens: R${male_median:.2f}", 
                               workbook.add_format({"color": male_color, "bold": True}))
        
        # Update offsets for next CBO
